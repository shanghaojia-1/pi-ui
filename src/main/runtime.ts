 ...
 * every separator (and the drive colon) into `-`, so paths containing literal
 * `-` are ambiguous — this is a display-only fallback for legacy sessions
 * whose JSONL header carries no cwd.
 */
const decodeSessionDirName = (name: string): string | null => {
  ...
  return restored.replace(/-/g, process.platform === 'win32' ? '\\' : '/')
}
